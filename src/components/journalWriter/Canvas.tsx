...
export default function Canvas({
  ...
  const setLeafOptionsFocus = (id:number, value:boolean) => {
    leafTb.updateField(id, 'optionsFocused', (_optionsFocused:boolean) => value)
    if (value)
      setFocusedId(0);
  };

  const handleMouseDownLeaf:(e: React.MouseEvent<SVGTextElement, MouseEvent>, leaf:LeafType) => void =
      (e, leaf) => {
    e.stopPropagation();
    ...
    setDrag({state:1, id:leaf.id, region:mouseoverRegion, offsetX:(x-leaf.x), offsetY:(y-leaf.y)});
  }
  ...
  const handleMouseDrag:React.MouseEventHandler = (e) => {
    const x = e.clientX;
    const y = e.clientY;
    ...
    <svg id="canvas"
      ...
      {leaves.map((_leaf) =>
        <Leaf
          ...
          handleMouseUp={(e) => handleMouseUpLeaf(e, _leaf.id)}
